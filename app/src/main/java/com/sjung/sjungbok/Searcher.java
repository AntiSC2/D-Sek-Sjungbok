...
public class Searcher extends AsyncTask<Void, ArrayList<Song>, ArrayList<Song>> {
	...
	public Searcher(Context context, MainActivity mainActivity, ArrayList<Song> allSongList, boolean titleSearch,
			boolean lyricsSearch, ArrayList<String> searchString) {
	...
	protected ArrayList<Song> doInBackground(Void... params) {
		...
		// bara lägga in de i sökresultaten om de matchar alla orden? (kanske bara lägga
		// in de som matchar alla orden om det finns någon sån match? annars ta del
		// matcher?)
		int score = 0;
		...
		for (int i = 0; i < allSongsList.size(); i++) {
			...
			if (score == allSongsList.get(i).titleAsArrayList().size() && titleSearch
					&& allSongsList.get(i).titleAsArrayList().size() == searchString.size()) {
		...
		if (songWithMatchingFullMatch.size() == 1) {
			if (perfectScoreMatch == songWithMatchingFullMatch.get(0).song.titleAsArrayList().size() && titleSearch
					&& songWithMatchingFullMatch.get(0).song.titleAsArrayList().size() == searchString.size()) {
		...
		Collections.sort(songWithMatching, new Comparator<SongAndMatching>() {
			...
			public int compare(SongAndMatching a, SongAndMatching b) {
				...
				if (a.matching == b.matching) {
					// inte a.song.compareTo(b.song) här eftersom vi sen vänder på allt innan vi
					// visar upp det(därför måste denna vara felvänd först)
